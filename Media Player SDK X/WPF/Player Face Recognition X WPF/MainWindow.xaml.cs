...
namespace Player_Face_Recognition_X_WPF
{
    /// <summary>
    /// Inserts a deep-learning face recognizer into MediaPlayerCoreX through the Video_Processing_AddBlock
    /// API: the recognizer taps the decoded video, draws labelled boxes into the rendered frame, and raises
    /// OnFacesIdentified (matched 1:N against an enrolled gallery) while the engine plays a normal file.
    /// </summary>
    public partial class MainWindow : Window
    {
        ...
        // Embedding family the current gallery was built with; null when empty or loaded from a .vfg.
        ...
        // Set while RebuildGalleryAsync re-embeds on a background thread; teardown drains it too.
        ...
        // Show a Download button only when the selected model is missing from the cache.
        ...
        {
            var dlg = new OpenFileDialog { Filter = "Media files|*.mp4;*.mkv;*.avi;*.mov;*.webm;*.ts|All files|*.*" };
        ...
        {
            // Lock the family selector during the download so a mid-download switch can't mismatch the weights.
        ...
        private async System.Threading.Tasks.Task DownloadModelAsync(Button button, TextBox target, string fileName)
        {
            ...
            try
            {
                ...
                // Download off the UI thread to a temp file, renaming into place only after it fully succeeds.
                    ...
                    using (var response = await _http.GetAsync(ModelsReleaseUrl + "/" + fileName, HttpCompletionOption.ResponseHeadersRead))
                    {
                        ...
                        using (var fileStream = File.Create(tmpPath))
                        {
                            ...
                            while ((read = await src.ReadAsync(buffer, 0, buffer.Length)) > 0)
                            {
                                ...
                                {
                                    // No Content-Length: throttle progress posts to whole-MB steps.
        ...
        private async void btEnroll_Click(object sender, RoutedEventArgs e)
        {
            ...
            // Bail if closing, already enrolling, or a Start/Stop transition is in flight.
            ...
            try
            {
                ...
                // Loaded gallery from a different family with no source photos to rebuild from: can't enroll.
                ...
                {
                    // Stop the batch if the window is closing.
                    if (_isClosing)
                    {
                        break;
                    }
                    ...
                    try
                    {
                        // Offload the heavy detection + embedding inference off the UI thread.
        ...
        // Re-embeds the gallery from the source photos when the embedding model changed since enrollment.
        ...
        // Clears and re-embeds every enrolled photo with the current model on a background thread.
        ...
        private void btSaveGallery_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (dlg.ShowDialog() == true)
            {
                try
                {
                    ...
                    // Persist the embedding family so a later Load can re-select the matching model.
        ...
        private async void btStart_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                MessageBox.Show(this, "Select a valid, existing media file on the Source tab.");
                return;
            }
            ...
            try
            {
                ...
                // Build the source before registering the block (a CreateAsync failure here must not strand one).
        ...
        /// <summary>
        /// Detaches the handler and drops the reference to the inserted recognizer; the engine disposes the block itself.
        /// </summary>
        ...
        private async void Window_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            // Drain an in-flight enroll/rebuild before disposing the shared enroller (use-after-free otherwise).
