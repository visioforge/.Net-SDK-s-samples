    ...
    public partial class MainWindow : Window
    {
        ...
        private async void btStart_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Create source
                var sourceResolution = _currentCamera.GetSensorSize();
                var frameRate = _currentCamera.GetFrameRate();
                _core.Video_Source = new GenICamSourceSettings(selectedCameraName, new VisioForge.Core.Types.Rect(0, 0, sourceResolution.Width, sourceResolution.Height), frameRate, GenICamPixelFormat.Default);

                if (isCaptureMode)
                {
                    // Capture to MP4 mode
                    string filename = tbFilename.Text.Trim();
                    _mp4Output = new MP4Output(filename, H264EncoderBlock.GetDefaultSettings(), null); // Uses default H264 encoders without audio

                    _core.Outputs_Add(_mp4Output);
                }
        ...
        private void cbCamera_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            {
                LoadCameraInformation(cbCamera.SelectedItem.ToString());
            }
        }
