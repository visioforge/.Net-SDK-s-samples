...
void main() {
    // Calculate the texel size (1.0 / texture width)
    ...
    // Apply horizontal Gaussian blur - unrolled loop for GLSL ES 2.0 compatibility
    float offset1 = 1.0 * blur_radius * texelSize;
    result += texture2D(tex, v_texcoord + vec2(offset1, 0.0)).rgb * WEIGHT1;
    result += texture2D(tex, v_texcoord - vec2(offset1, 0.0)).rgb * WEIGHT1;
    
    float offset2 = 2.0 * blur_radius * texelSize;
    result += texture2D(tex, v_texcoord + vec2(offset2, 0.0)).rgb * WEIGHT2;
    result += texture2D(tex, v_texcoord - vec2(offset2, 0.0)).rgb * WEIGHT2;
    
    float offset3 = 3.0 * blur_radius * texelSize;
    result += texture2D(tex, v_texcoord + vec2(offset3, 0.0)).rgb * WEIGHT3;
    result += texture2D(tex, v_texcoord - vec2(offset3, 0.0)).rgb * WEIGHT3;
    
    float offset4 = 4.0 * blur_radius * texelSize;
    result += texture2D(tex, v_texcoord + vec2(offset4, 0.0)).rgb * WEIGHT4;
    result += texture2D(tex, v_texcoord - vec2(offset4, 0.0)).rgb * WEIGHT4;
