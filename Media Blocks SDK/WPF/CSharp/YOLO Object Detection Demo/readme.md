# Media Blocks SDK .Net - Object Detection & Analytics Demo (C#/WPF)
...
detection with its class label and confidence. In **Object analytics** mode it adds multi-object tracking
with stable IDs, tripwire line crossing, and polygon zone occupancy.
...
* `ObjectAnalyticsBlock` - Multi-object tracking, tripwire, polygon zones, and overlay rendering
  (used in analytics mode instead of `YOLOObjectDetectorBlock`).
...
**Analytics mode pipeline:**

```mermaid
graph LR
    Source[Webcam, RTSP camera, or File] -- video --> ObjectAnalyticsBlock -- video --> VideoRendererBlock
```
...
2. On the **Detector** tab pick the **Model family**. The bundled `yolox_nano.onnx` is selected by default;
   pick a different `.onnx` model with the **Download** or **Browse...** button.
3. On the **Analytics** tab pick the mode: **Object detection** (ordinary detector) or
   **Object analytics** (tracking + tripwire + polygon zones).
4. Press **Start**.
...
### Object analytics mode

When **Object analytics** is selected on the **Analytics** tab:

* The detector runs with a lowered confidence threshold (the ByteTrack low-confidence threshold)
  so the tracker can associate low-confidence detections with existing tracks.
* A **ByteTrack** multi-object tracker assigns stable integer IDs to each tracked object.
* A directed **tripwire line** counts objects crossing from one side to the other (`In`/`Out`).
  Coordinates are source-frame pixels; the direction is `Start → End`.
* A **polygon zone** tracks which objects are currently inside a closed region.
  `Current count` includes lost tracks that were last seen inside (surviving short detector dropouts).
* Overlays: colored boxes (stable per-track color), labels, track IDs, traces, line, polygon, and counters.
* Track age is measured in processed inference frames, not wall-clock time.
* `FramesToSkip` reduces inference frequency but does not create predicted intermediate frames.
* Counters and traces reset on stop/restart.
