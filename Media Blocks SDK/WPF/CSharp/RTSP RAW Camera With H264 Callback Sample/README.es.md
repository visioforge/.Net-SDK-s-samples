...
```mermaid
graph LR
    subgraph Grabber Pipeline
        RTSPRAWSourceBlock -- video --> BufferSinkBlock
    end
    subgraph Player Pipeline
        H264PushSourceBlock --> H264ParseBlock --> H264DecoderBlock --> VideoRendererBlock
    end
    BufferSinkBlock -.-> |PushData| H264PushSourceBlock
```

## Bloques de medios utilizados

* `RTSPRAWSourceBlock` - Captura de flujo RTSP sin procesar
* `BufferSinkBlock` - Callback de buffer de datos sin procesar
* `H264PushSourceBlock` - Fuente push de datos H.264 (AppSrc)
* `H264ParseBlock` - Analizador de flujo H.264
* `H264DecoderBlock` - Decodificador de video H.264
